...
const BG_COLORS = [
  '#2E5C6E', '#C75B39', '#B8860B', '#4A8068', '#5B7B8A', '#7B5EA7', '#121214', '#ffffff', '#F5F5F5', '#1C1C1E'
];
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-bg text-text transition-colors duration-300 selection:bg-primary/30">
      {/* Header */}
      <header className="sticky top-0 z-50 bg-bg/80 backdrop-blur-xl border-b border-border px-6 py-3">
        <input 
          type="file" 
          ref={fileInputRef}
        ...
        <div className="max-w-[1600px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-primary rounded-xl flex items-center justify-center shadow-lg shadow-primary-dark/20">
              <Layers className="text-white w-6 h-6" />
            </div>
            <div>
              <h1 className="text-lg font-bold tracking-tight text-text">ASO Studio <span className="text-primary-light">Pro</span></h1>
              <p className="text-[9px] text-text-secondary font-medium uppercase tracking-wider">Professional Asset Forge</p>
            </div>
          </div>
      ...
      <main className="max-w-[1600px] mx-auto p-4 lg:p-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <aside className="lg:col-span-3 space-y-4">
            ...
            <section className="glass-panel p-4 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-bold text-[10px] uppercase tracking-wider">App Icon</h2>
                </div>
                <div className="flex items-center gap-2">
                  {appIcon && (
                    <button 
                      onClick={() => setAppIcon(null)}
                      className="text-[9px] font-bold text-red-400/60 hover:text-red-400 transition-all"
                    >
                      REMOVE
                    </button>
                  )}
                  <button 
                    ...
                    className={cn(
                      ...
                      globalShowIcon ? "bg-primary/20 text-primary-light" : "bg-surface-variant text-text-tertiary"
                    )}
                  >
              ...
              <div 
                ...
                className="w-16 h-16 mx-auto rounded-2xl border-2 border-dashed border-border flex flex-col items-center justify-center cursor-pointer hover:border-primary/50 hover:bg-surface-variant transition-all group overflow-hidden relative"
              >
                {appIcon ? (
                  <>
                  ...
                  <>
                    <Plus className="w-4 h-4 text-text-tertiary group-hover:text-primary transition-all" />
                  </>
                )}
                <input 
                  type="file" 
            ...
            <section className="glass-panel p-4 space-y-3 border-primary/10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-primary" />
                  <h2 className="font-bold text-[10px] uppercase tracking-wider">AI Copywriter</h2>
                </div>
                <button 
                  onClick={() => setShowPrompt(!showPrompt)}
                  className="text-[9px] font-bold text-primary-light hover:text-primary transition-all bg-primary/10 px-1.5 py-0.5 rounded"
                >
                  {showPrompt ? 'HIDE' : 'GET'}
                </button>
              </div>
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                  <div className="p-4 rounded-xl bg-primary/5 border border-primary/20 space-y-3">
                    <p className="text-[11px] text-white/80 leading-relaxed font-medium">
                      Copy this prompt and use it with your screenshots in Gemini or ChatGPT:
                    </p>
                    <div className="p-3 rounded-lg bg-surface-variant border border-border relative group">
                      <p className="text-[10px] text-text-secondary leading-relaxed pr-8 italic select-all">
                        "based on metadata,first capture image will first image,for all {screenshots.length} images in sequence give me engaging headline and subheadline format will be

headline $- subheadline
headline2 $- subheadline2
so on"
                      </p>
                    </div>
                    <button 
                      onClick={() => {
                        const prompt = `based on metadata,first capture image will first image,for all ${screenshots.length} images in sequence give me engaging headline and subheadline format will be\n\nheadline $- subheadline\nheadline2 $- subheadline2\nso on`;
                        navigator.clipboard.writeText(prompt);
                        confetti({ particleCount: 60, spread: 70, origin: { y: 0.8 }, colors: ['#2E5C6E', '#ffffff', '#4A8FA3'] });
                      }}
                      className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-primary text-white text-[11px] font-bold hover:bg-primary-light transition-all shadow-lg shadow-primary/20"
                    >
                  ...
                  <div className="flex items-start gap-3 p-3 rounded-lg bg-surface-variant border border-border">
                    <div className="w-5 h-5 rounded-full bg-primary/20 flex items-center justify-center flex-shrink-0 text-[10px] font-bold text-primary-light">!</div>
                    <p className="text-[10px] text-text-secondary leading-relaxed">
                      After the AI generates the text, copy the entire list and paste it into the <b>Bulk Import</b> area below.
                    </p>
                  </div>
                </div>
              )}
            ...
            <section className="glass-panel p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-bold text-[10px] uppercase tracking-wider">Bulk Import</h2>
                </div>
                {bulkText && (
                  <button 
                    onClick={() => {
                    ...
                    className="text-[9px] font-bold text-text-tertiary hover:text-red-400 transition-all"
                  >
                    CLEAR
                  </button>
                )}
              ...
              <div className="relative group">
                <textarea
                  className={cn(
                    "control-input min-h-[160px] text-[11px] py-4 px-4 resize-none transition-all border-dashed border-2",
                    bulkText ? "border-primary/30 bg-primary/5" : "border-border hover:border-primary/40"
                  )}
                  placeholder="PASTE AI OUTPUT HERE...&#10;&#10;Example:&#10;Fast Performance $- Speed up your workflow&#10;Secure Data $- Your privacy matters"
                  value={bulkText}
                  onPaste={(e) => {
                    ...
                    setScreenshots(prev => prev.map((s, i) => {
                      if (lines[i]) {
                        // Try multiple separators: $- (preferred), then &- then : then -
                        let h = '', sh = '';
                        if (lines[i].includes('$-')) {
                    ...
                    confetti({ particleCount: 30, spread: 50, origin: { y: 0.9 }, colors: ['#10b981'] });
                  }}
                  onChange={(e) => {
                ...
                />
                {!bulkText && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none opacity-20 group-hover:opacity-40 transition-all">
                    <Clipboard className="w-8 h-8 mb-2" />
                    <span className="text-[10px] font-bold tracking-widest uppercase">Paste Zone</span>
                  </div>
                )}
              </div>
              
              <div className="flex items-center justify-between text-[9px] text-white/30 italic">
                <div className="flex items-center gap-2">
                  <Check className="w-3 h-3 text-primary" />
                  <span>Format: Headline &- Subheadline (one per line)</span>
                </div>
                {bulkText && (
                  <div className={cn(
                    "font-bold uppercase tracking-widest",
                    bulkText.split('\n').filter(l => l.trim()).length >= screenshots.length ? "text-emerald-500" : "text-amber-500"
                  )}>
                    {bulkText.split('\n').filter(l => l.trim()).length} / {screenshots.length} Lines
                  </div>
                )}
              </div>
            </section>
            ...
            <footer className="pt-8 pb-4 border-t border-border">
              <div className="flex flex-col items-center gap-2">
                <p className="text-[10px] text-text-tertiary font-medium">© 2024 Shaad Dev Studio</p>
      ...
      {/* Footer */}
      <footer className="mt-20 border-t border-border py-12 px-6 bg-surface/50">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-surface-variant rounded-lg flex items-center justify-center border border-border">
              <Smartphone className="w-4 h-4 text-text-secondary" />
            </div>
            <span className="font-bold text-lg tracking-tight text-text">ASO Studio <span className="text-primary-light">Pro</span></span>
          </div>
          <p className="text-xs text-text-secondary font-medium tracking-wide">© 2024 Shaad Dev Studio. All rights reserved.</p>
          <div className="flex items-center gap-8">
            <a href="#" className="text-[10px] font-bold text-text-tertiary hover:text-primary transition-all uppercase tracking-widest">Privacy Policy</a>
            <a href="#" className="text-[10px] font-bold text-text-tertiary hover:text-primary transition-all uppercase tracking-widest">Terms of Service</a>
            <a href="#" className="text-[10px] font-bold text-text-tertiary hover:text-primary transition-all uppercase tracking-widest">Enterprise Support</a>
          </div>
        </div>
      </footer>
