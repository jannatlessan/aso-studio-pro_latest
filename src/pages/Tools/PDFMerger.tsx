...
export default function PDFMerger() {
  ...
  // Smart Navigation
  const isToolUsed = files.length > 0;
  const resetAll = () => {
    setFiles([]);
    setMergedUrl(null);
      ...
      <nav className="sticky top-0 z-50 bg-white/85 backdrop-blur-xl border-b border-black/[0.06] px-4 sm:px-8 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <button onClick={handleBackClick} className="flex items-center gap-3 group text-[#55605B] hover:text-primary transition-colors" title={isToolUsed ? "(Click to reset)" : undefined}>
            ...
            <span className="font-bold tracking-wider text-sm uppercase hidden sm:inline">{isToolUsed ? 'PDF Merger' : 'Back to Tools'}</span>
          </button>
      ...
      <main className="flex-grow p-4 sm:p-8 flex flex-col items-center">
        <div className="w-full max-w-4xl space-y-6">
          <div className="space-y-1 text-center sm:text-left">
             <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-ink">Merge PDF Documents</h1>
             <p className="text-[#55605B] text-sm">Combine multiple PDFs smoothly right in your browser. Complete privacy.</p>
          </div>
          ...
          <div className="p-6 sm:p-8 rounded-2xl bg-white border border-black/10 shadow-lg shadow-black/5 space-y-6">
            ...
            <div
              ...
              className="border-2 border-dashed border-black/10 hover:border-primary/50 text-[#8B958F] hover:text-primary rounded-xl flex flex-col items-center justify-center p-8 sm:p-12 cursor-pointer transition-all bg-[#F6F8F7]"
            >
              <input 
                type="file" 
            ...
            {files.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center gap-2 border-b border-black/10 pb-3">
                  ...
                  <h2 className="font-black text-xs uppercase tracking-widest text-[#55605B]">Documents Queue ({files.length})</h2>
                </div>
                  ...
                  {files.map((f, index) => (
                    <div key={f.id} className="flex items-center justify-between p-3 bg-[#F6F8F7] border border-black/[0.06] rounded-lg hover:border-black/10 group transition-all">
                      <div className="flex items-center gap-3 truncate">
                        ...
                        <span className="text-[10px] uppercase bg-black/[0.04] px-2 py-0.5 rounded text-[#8B958F] border border-black/10">
                ...
                <div className="pt-4 flex justify-between items-center">
                  <span className="text-xs text-[#8B958F]">Output: Single Merged PDF</span>
                  <button
                    onClick={mergePDFs}
                    disabled={isProcessing || files.length < 2}
                    className="px-6 py-3 bg-primary-dark hover:bg-[#048532] text-white disabled:opacity-50 disabled:cursor-not-allowed rounded-xl font-black uppercase tracking-widest text-xs transition-all flex items-center gap-2 shadow-lg shadow-primary/25"
                  >
                    {isProcessing ? (
            ...
            {mergedUrl && (
               <div className="mt-8 p-6 bg-mint border border-primary/20 rounded-xl space-y-4">
                 ...
                 <a
                   ...
                   className="inline-flex w-full sm:w-auto mt-2 px-6 py-3 bg-primary-dark hover:bg-[#048532] text-white rounded-lg font-bold uppercase tracking-widest text-xs transition-all justify-center items-center gap-2"
                 >
                   Download Merged PDF
                 </a>
               </div>
          ...
          <article className="p-6 sm:p-8 rounded-2xl bg-white border border-black/10 shadow-sm shadow-black/[0.02] mt-12 space-y-6 text-sm text-[#55605B] leading-relaxed font-sans">
            <h2 className="text-xl sm:text-2xl font-semibold text-ink">Secure Client-Side PDF Document Merger</h2>
            <p>
            ...
            <div className="space-y-4">
              <div>
                <strong className="text-ink block">1. Are my sensitive legal or financial PDFs secure?</strong>
              ...
              <div>
                <strong className="text-ink block">2. Do I lose the original text searchability within my merged PDF?</strong>
              ...
              <div>
                <strong className="text-ink block">3. Is there a hard limit on the number of PDFs I can merge?</strong>
          ...
          <section className="mt-12 mb-6"><h3 className="text-2xl font-bold text-ink font-mono border-b border-black/10 pb-4 mb-6">Best Practices for Usage & SEO</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="space-y-4 rounded-lg bg-[#F6F8F7] p-6 border border-black/10"><h4 className="font-bold text-ink mb-2">Logical Sequencing</h4><p className="text-[#55605B]">Ensure the page order makes chronological sense to tell a cohesive document narrative.</p></div><div className="space-y-4 rounded-lg bg-[#F6F8F7] p-6 border border-black/10"><h4 className="font-bold text-ink mb-2">Optimize Source Docs</h4><p className="text-[#55605B]">Pre-compress huge individual PDFs before merging them together to avoid bloat.</p></div></div></section></article>
