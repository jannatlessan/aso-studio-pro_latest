...
export default function ToolsList() {
  ...
  return (
    <div className="relative min-h-screen bg-[#08080A] text-[#EDEDEF] selection:bg-primary/20 overflow-x-hidden">
      <SEO
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 md:px-10 pt-32 pb-16 space-y-10" ref={containerRef}>
        ...
        <div className="space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/[0.04] border border-white/10 text-xs font-medium text-white/65 w-fit">
        ...
        <div className="space-y-3">
          <div className="relative max-w-2xl">
            ...
            <input
              ref={searchInputRef}
              type="text"
              className="w-full bg-white/[0.03] border border-white/10 rounded-xl py-3 pl-11 pr-14 text-sm text-white placeholder-white/30 focus:border-primary/50 focus:ring-1 focus:ring-primary/50 focus:outline-none transition-all"
          ...
          {(searchQuery || selectedCategory) && (
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className="text-xs px-2.5 py-1 rounded-full border border-white/10 hover:border-primary/40 bg-white/[0.03] hover:bg-primary/10 text-white/60 hover:text-primary transition-all font-medium"
              >
                Clear filters
              </button>
            </div>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all border ${
                  selectedCategory === category
                    ...
                    : 'border-white/10 bg-white/[0.03] text-white/60 hover:border-primary/30 hover:bg-white/[0.06] hover:text-white/80'
                }`}
              >
                {category}
              </button>
        ...
        {filteredTools.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-left">
            {filteredTools.map((tool) => (
              <Link
                ...
                key={tool.id}
                className={`block tool-card group p-5 border rounded-2xl transition-all duration-300 relative overflow-hidden ${tool.status === 'Ready' || tool.status === 'Trending' || tool.status === 'New' ? 'border-white/10 bg-white/[0.02] hover:border-primary/30 hover:bg-white/[0.04] cursor-pointer' : 'border-white/10 bg-white/[0.015] cursor-not-allowed'}`}
              >
                <div className="relative z-10 space-y-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className={`px-2.5 py-1 border rounded-full text-[10px] font-medium inline-flex items-center gap-1 ${tool.status === 'Ready' || tool.status === 'Trending' || tool.status === 'New' ? 'border-primary/30 text-primary bg-primary/10' : 'border-white/15 text-white/50 bg-white/[0.03]'}`}>
                  ...
                  <div className="pt-4 border-t border-white/[0.06]">
                    {tool.status === 'Ready' || tool.status === 'Trending' || tool.status === 'New' ? (
                      ...
                      <span className="inline-flex items-center gap-1.5 text-white/45 font-semibold text-xs">
                        {tool.actionLabel}
                      </span>
                    )}
                  </div>
          ...
          <div className="py-8 text-center space-y-4">
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-white/[0.04] border border-white/10">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/15 hover:bg-primary/25 border border-primary/40 text-primary font-semibold text-xs transition-all"
            >
              Clear all filters
            </button>
          </div>
        ...
        <div className="p-4 border border-white/10 rounded-2xl bg-white/[0.02] text-xs text-white/60 leading-relaxed">
