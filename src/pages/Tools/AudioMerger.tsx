      ...
      <main className="flex-grow p-4 sm:p-8 flex flex-col items-center">
        <div className="w-full max-w-4xl space-y-6">
          <div className="space-y-1 text-center sm:text-left">
             <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-ink">Merge Audio Files</h1>
             <p className="text-[#55605B] text-sm">Combine MP3s & WAVs seamlessly right in your browser. 100% private and offline.</p>
          </div>

          <div className="p-6 sm:p-8 rounded-2xl bg-white border border-black/10 shadow-lg shadow-black/5 space-y-6">

            <div
              onClick={() => fileInputRef.current?.click()}
              className="border-2 border-dashed border-black/15 hover:border-primary/50 text-[#8B958F] hover:text-primary rounded-xl flex flex-col items-center justify-center p-8 sm:p-12 cursor-pointer transition-all bg-[#F6F8F7]"
            >
              <input
                type="file"
                accept="audio/*"
              ...
              <span className="text-xs opacity-70 mt-1">Select multiple files (MP3, WAV, OGG)</span>
            </div>
            ...
            {files.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center gap-2 border-b border-black/10 pb-3">
                  ...
                  <h2 className="font-black text-xs uppercase tracking-widest text-[#55605B]">Audio Tracks ({files.length})</h2>
                </div>

                <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                  {files.map((f, index) => (
                    <div key={f.id} className="flex items-center justify-between p-3 bg-[#F6F8F7] border border-black/[0.06] rounded-lg hover:border-black/10 group transition-all">
                      <div className="flex items-center gap-3 truncate">
                        <span className="text-xs font-black text-[#8B958F] w-4">{index + 1}.</span>
                        <span className="text-sm truncate text-ink">{f.name}</span>
                        <span className="text-[10px] uppercase bg-white px-2 py-0.5 rounded text-[#8B958F] border border-black/[0.06]">
                          {(f.file.size / 1024 / 1024).toFixed(2)} MB
                        </span>
                      </div>
                      <button
                        onClick={() => removeFile(f.id)}
                        className="text-[#8B958F] hover:text-red-500 p-1 rounded transition-colors"
                        title="Remove track"
                      >
                ...
                <div className="pt-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-[#55605B] uppercase tracking-widest font-bold">Output Format:</span>
                    <select
                      value={outputFormat}
                      onChange={(e) => setOutputFormat(e.target.value as 'mp3' | 'wav')}
                      className="bg-white border border-black/10 rounded-lg px-3 py-1.5 text-xs text-ink focus:border-primary focus:outline-none uppercase tracking-wider"
                    >
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto px-6 py-3 bg-primary-dark text-white hover:bg-[#048532] disabled:opacity-50 disabled:cursor-not-allowed rounded-xl font-black uppercase tracking-widest text-xs transition-all flex items-center justify-center gap-2 shadow-lg shadow-primary/25"
                  >
                    {isProcessing ? (
            ...
            {mergedUrl && (
               <div className="mt-8 p-6 bg-mint border border-primary/20 rounded-xl space-y-4">
                 ...
                 <a
                   ...
                   className="inline-flex w-full sm:w-auto mt-2 px-6 py-3 bg-primary-dark text-white hover:bg-[#048532] rounded-full font-bold uppercase tracking-widest text-xs transition-all justify-center items-center gap-2 shadow-lg shadow-primary/25"
                 >
          ...
          <article className="p-6 sm:p-8 rounded-2xl bg-white border border-black/10 shadow-sm shadow-black/[0.02] mt-12 space-y-6 text-sm text-[#55605B] leading-relaxed font-sans">
            <h2 className="text-xl sm:text-2xl font-semibold text-ink">Seamless Audio File Merger & Joiner</h2>
            <p>
            ...
            <div className="space-y-4">
              <div>
                <strong className="text-ink block">1. What audio formats do you support for merging?</strong>
              ...
              <div>
                <strong className="text-ink block">2. Is there a noticeable gap or pop between merged tracks?</strong>
              ...
              <div>
                <strong className="text-ink block">3. Are my private voice recordings stored on your servers?</strong>
          ...
          <section className="mt-12 mb-6"><h3 className="text-2xl font-bold text-ink font-mono border-b border-black/10 pb-4 mb-6">Best Practices for Usage & SEO</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="space-y-4 rounded-lg bg-[#F6F8F7] p-6 border border-black/10"><h4 className="font-bold text-ink mb-2">Consistent Formats</h4><p className="text-[#55605B]">For the cleanest crossfades, use source audio files with matching sample rates.</p></div><div className="space-y-4 rounded-lg bg-[#F6F8F7] p-6 border border-black/10"><h4 className="font-bold text-ink mb-2">Sequence Order</h4><p className="text-[#55605B]">Always double check the sequence order before downloading the final mp3 file.</p></div></div></section></article>
